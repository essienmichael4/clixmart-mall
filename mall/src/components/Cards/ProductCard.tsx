...
const ProductCard = React.forwardRef(({product, ref }: ProductProps,  ) => {
    ...
    const contains = cartItems.some(item=> item.id ===product.id)
        ...
        <>
            <div className='border p-2 rounded-md'>
                <div className='relative'>
                    ...
                    {!contains && <button onClick={()=> increaseCartQuanity(product!.id)} className='absolute -bottom-5 right-4 rounded-full bg-blue-800 p-2 border-2 border-white hover:bg-blue-400'><ShoppingCart className='w-4 h-4 sm:w-5 sm:h-5 text-white'/></button>}
                </div>
