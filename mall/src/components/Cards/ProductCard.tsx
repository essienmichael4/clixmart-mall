...
const ProductCard = React.forwardRef(({product, ref }: ProductProps,  ) => {
    ...
    const productBody = (
        <>
            <Link to={`products/${product.productId}`} className='border p-2 block overflow-hidden rounded-md relative'>
    ...
    const content = ref ? 
        <div ref={ref} 
        className='p-1 sm:p-2 w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/5 '
        >{productBody}</div>
        :
        <div 
        className='p-1 sm:p-2 w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/5'
