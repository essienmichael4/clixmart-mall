...
const StoreNextOfKinDetails = ({store, id, formStep, setFormStep}:Props) => {
    const [phone, setPhone] = useState(store?.nextOfKin?.phone || "")
    ...
    const form = useForm<NextOfKinInfoSchemaType>({
        ...
        defaultValues:{
            name: store?.nextOfKin?.name || "",
            phone: store?.nextOfKin?.phone || "",
        }
    })

    const addNextOfKin = async (data:NextOfKinInfoSchemaType)=>{
        const response = await axios_instance_token.patch(`/stores/${id}/next-of-kin`, {
            ...data
        },)
    ...
    const onSubmit = async (data:NextOfKinInfoSchemaType) =>{
        toast.loading("Updating store payment details...", {
            id: "create-store"
        })
        mutate(data)
    }
