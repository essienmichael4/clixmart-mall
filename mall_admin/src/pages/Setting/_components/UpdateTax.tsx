...
import { EditTaxSchema, EditTaxSchemaType } from '@/schema/tax'
...
const UpdateTax = ({trigger, tax}:Props) => {
    const [open, setOpen] = useState(false)
    const axios_instance_token = useAxiosToken()
    const queryClient = useQueryClient()

    const form = useForm<EditTaxSchemaType>({
    ...
    const addTax = async (data:EditTaxSchemaType)=>{
        const response = await axios_instance_token.patch(`/settings/tax/${tax.id}`, {
            ...data
        },)
    ...
    const onSubmit = (data:EditTaxSchemaType)=>{
        toast.loading("Updating tax...", {
            id: "update-tax"
        })
        mutate(data)
    }
