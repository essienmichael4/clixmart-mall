...
const useProductsSearch = ({q, page = 1, category, subCategory, take=50, brand}:ProductSearchParams) => {
    ...
    const productsQuery = useQuery<ProductsResponse>({
        ...
        queryFn: async() => await axios_instance.get(`/products?q=${q || ""}&page=${page}&take=${take}&category=${category || ""}&subCategory=${subCategory || ""}&brand=${brand}`).then(res => {
            ...
            setIsLoading(false)
            if(query == q && prevCategory == category && subCategory == prevSubCategory && prevBrand == brand){
