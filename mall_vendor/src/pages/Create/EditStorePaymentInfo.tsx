...
const StorePaymentInfo = ({store, id, formStep, setFormStep}:Props) => {
    ...
    const form = useForm<StorePaymentInfoSchemaType>({
        ...
        defaultValues:{
            paymentMode: store?.paymentDetail?.paymentMode || "BANK",
            accountName: store?.paymentDetail?.accountName || "",
            accountNumber: store?.paymentDetail?.accountNumber || "",
            provider: store?.paymentDetail?.provider || ""
        }
    })
    ...
    const addStorePayment = async (data:StorePaymentInfoSchemaType)=>{
        const response = await axios_instance_token.patch(`/stores/${id}/payment-details`, {
            ...data
        },)
    ...
    const {mutate, isPending} = useMutation({
        ...
        onSuccess: ()=>{
            toast.success("Store payment updated successfully", {
                id: "create-store"
            })
