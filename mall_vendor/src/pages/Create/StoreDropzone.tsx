...
const StoreDropzone = ({handleFileChange, title}:Props) => {
    ...
    const file = files?.map((file, i)=> <img key={i} className='w-100%'  src={file.preview} alt={file.name} />).filter((_e , i)=> i == 0)
      ...
      return (
        <div className="flex w-[100%] flex-col mt-4 gap-2">
            <p className='text-xs 2xl:text-sm font-bold'>{title}</p>
            
            {!file && <div
                {...getRootProps({
                className:
                    'dropzone dropzone-border border border-dashed relative overflow-hidden rounded-xl bg-transparent p-10 cursor-pointer w-[60%] flex flex-col gap-y-4 items-center aspect-video mx-auto justify-center',
                })}
            >
                <input {...getInputProps()} />
            ...
            {file && <div className='w-[60%] mx-auto relative'>
